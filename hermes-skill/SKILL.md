...
1. **Ensure the CLI is installed.** Check `command -v linkedin-jobs`.
   - **Missing → run the bundled installer**, which is the single source of truth for the install logic. It downloads the pinned release binary and verifies it against the release's `checksums.txt` (sha256) plus a cosign keyless signature before executing — a tampered or unverified binary is refused:
     ```
     bash ~/.claude/skills/linkedin-jobs/scripts/install-cli.sh
     ```
     For other agents, substitute that agent's skill directory (e.g. `~/.agents/skills/linkedin-jobs/scripts/install-cli.sh`); the script ships alongside this `SKILL.md`. Its version pin is kept in sync with the `version:` at the top of this file by the `sync-version-pin` release job. The installer exits `0` on success; an exit code of `2` means the binary installed but isn't on `PATH` yet — relay the `export PATH="$HOME/.local/bin:$PATH"` hint it prints and start a new shell. **Do not proceed until `linkedin-jobs version` succeeds.**
